...
CUSTOM_PLATFORM = "linux" if SYS_PLATFORM in ["linux", "linux2"] else "mac" if SYS_PLATFORM in ["darwin"] else "windows"
...
ContaminantSearchResult = collections.namedtuple(
    "ContaminantSearchResult", ["pdb_code", "space_group", "uniprot_name", "uniprot_mnemonic"]
)
...
def create_lattice_db(database):
    ...
    url = (
        "http://www.rcsb.org/pdb/rest/customReport.csv?pdbids=*&customReportColumns=lengthOfUnitCellLatticeA,"
...
def create_contaminant_db(database, add_morda_domains, nproc=2, submit_qtype=None, submit_queue=False):
    ...
    if len(results) == 0:
        ...
        if add_morda_domains:
            ...
            logger.info(
                "%d new entries were found in the contaminant database, " + "updating SIMBAD database", len(results)
            )
            ...
            if morda_installed_through_ccp4:
                for dat_file in morda_dat_files:
                    if result.pdb_code.lower() == dat_file[0:4]:
                        stem = os.path.join(
                            database, result.uniprot_mnemonic, result.uniprot_name, result.space_group, "morda"
                        )
                        if not os.path.exists(stem):
                            os.makedirs(stem)
...
def create_ensemble_db(database, pdb_db, nproc=2, submit_qtype=None, submit_queue=False, chunk_size=5000):
    ...
    if len(dat_files) < 1:
        ...
        logger.info(
            "%d new entries were found in the MoRDa database, " + "updating SIMBAD ensemble database", len(dat_files)
        )
        ...
        for f in chunk_dat_files:
            ...
            cmd = [
                ...
                [
                    ...
                    "-c",
                    "'import simbad.util; "
                    'simbad.util.get_sequence("{0}", "{1}")\''.format(get_model_output, get_seq_output),
                ],
                ...
                [
                    ...
                    "-c",
                    "'import simbad.util; "
                    'simbad.util.get_mrbump_ensemble("{0}", "{1}")\''.format(mrbump_directory, final_file),
                ],
            ]
...
def create_db_custom(custom_db, database):
    ...
    custom_dat_files = set(
        [
            ...
            for root, _, files in os.walk(custom_db)
...
def create_db_argparse():
    ...
    pc.add_argument(
        "-simbad_db", type=str, default=simbad.MORDA_MODELS, help="Path to local copy of the SIMBAD database"
    )
