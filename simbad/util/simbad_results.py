...
class SimbadResults(object):
    ...
    lattice_results : :Pandas dataframe:
        Dataframe containing the results of the lattice search
    lattice_mr_results : :Pandas dataframe:
        Dataframe containing the results of the MR
    ...
    def __init__(self, work_dir):
        ...
        self.contaminant_mr_results = pd.read_csv(contaminant_mr_results) if os.path.isfile(contaminant_mr_results) else None
    ...
    def get_files(self, search_type, num_results=10):
        ...
        else: raise RuntimeError("Unrecognised search type: %s" % search_type)
        ...
        for i in range(0, num_results):
            try:
                ...
                mr_workdir = os.path.join(
                    self.work_dir, search_type, 'mr_search', pdb_code, 'mr', mr_program)
                fc.mr_log = os.path.join(
                    mr_workdir, '{0}_mr.log'.format(pdb_code))
                fc.ref_pdb = os.path.join(
                    mr_workdir, 'refine', '{0}_refinement_output.pdb'.format(pdb_code))
                fc.ref_pdb_annotation = 'PDB #{0} from REFMAC-refined result of the {1} search'.format(i+1, ID2STR[search_type])
                fc.ref_mtz = os.path.join(
                    mr_workdir, 'refine', '{0}_refinement_output.mtz'.format(pdb_code))
                fc.ref_mtz_annotation = 'MTZ #{0} from REFMAC-refined result of the {1} search'.format(i+1, ID2STR[search_type])
                fc.ref_log = os.path.join(
                    mr_workdir, 'refine', '{0}_ref.log'.format(pdb_code))
                fc.ref_map = os.path.join(
                    mr_workdir, 'refine', '{0}_refmac_2fofcwt.map'.format(pdb_code))
                fc.diff_map = os.path.join(
